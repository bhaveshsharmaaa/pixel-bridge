    ...
    <p className="self-stretch font-bold font-montserrat mt-10 text-xl text-white max-md:mt-10 max-md:max-w-full">
      ...
      {featureItems.slice(0, 4).map((item, index) => (
        <React.Fragment key={index}>
          <span className="text-[#A4A8FF] font-montserrat">{item}</span>
          {index < 3 && ", "}
        </React.Fragment>
      ))}
      .<br />
      <br />
      Use the Alabay's{" "}
      <span className="text-[#A4A8FF] font-montserrat">
        {featureItems[4]}
      </span>{" "}
      to solve{" "}
      <span className="text-[#A4A8FF] font-bold font-montserrat">
        {featureItems[5]}
      </span>{" "}
      ...
      Challenge Modes:
      <br />
      <br />- <span className="text-[#A4A8FF]">{featureItems[6]}</span>: Compete
      in special timed challenges where the Alabai must navigate through complex
      mazes or
      <br />
      complete puzzles within a time limit.
      <br />
      <br />- <span className="text-[#A4A8FF]">{featureItems[7]}</span>: A mode
